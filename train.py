...
from utils import train_transform, style_transform, denormalize, gram_matrix
...
if __name__ == "__main__":
    ...
    if torch.backends.mps.is_available():
        ...
        print("🔋 Používá se MPS (Metal Performance Shaders)")
    else:
        device = torch.device("cpu")
        print("⚠️ MPS není dostupné – používá se CPU")
    ...
    image_paths = glob.glob(f"{args.dataset_path}/*/*.png") + glob.glob(f"{args.dataset_path}/*/*.jpg") + glob.glob(f"{args.dataset_path}/*/*.jpeg")
    sample_count = min(8, len(image_paths))
    image_samples = []
    for path in random.sample(image_paths, sample_count):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_i, (images, _) in enumerate(dataloader):
            ...
            style_loss = 0
            for ft_y, gm_s in zip(features_transformed.values(), gram_style):
